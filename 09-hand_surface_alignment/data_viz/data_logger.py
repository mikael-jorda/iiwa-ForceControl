...
EE_SENSED_FORCE_LOGGER_KEY = "sai2::iiwaForceControl::logger::force_moment";
EE_POSORI_CONTROL_FORCE_LOGGER_KEY = "sai2::iiwaForceControl::logger::ori_task_force";
...
while(runloop):
	...
	force = json.loads(r_server.get(EE_SENSED_FORCE_LOGGER_KEY))
	ori_task_force = json.loads(r_server.get(EE_POSORI_CONTROL_FORCE_LOGGER_KEY))

	# line = r_server.get(SIM_TIMESTAMP_KEY) + '\t' + " ".join([str(x) for x in force]) + '\t' + " ".join([str(x) for x in desired_force]) + '\n'
	line = '0\t' + " ".join([str(x) for x in force]) + '\t' + " ".join([str(x) for x in ori_task_force]) + '\n'
	# print line
