...
// simulation loop
void simulation(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* plate, Simulation::Sai2Simulation* sim);
...
void simulation(Sai2Model::Sai2Model* robot, Sai2Model::Sai2Model* plate, Simulation::Sai2Simulation* sim)
{
	...
	Eigen::Vector3d sensed_force_global, sensed_moment_global;
	...
	while (runloop) {
		...
		sim->setJointTorques(robot_name, robot_torques + disturbance_torque);
		...
		force_sensor->getForce(sensed_force_global);
		force_sensor->getMomentLocalFrame(sensed_moment);
		force_sensor->getMoment(sensed_moment_global);
		...
		if(sim_counter % 500 == 0)
		{
			...
			// std::cout << "end effector sensor moments : \t" << -sensed_forces_moments.tail(3).transpose() << std::endl;
			// std::cout << "end effector sensor forces global : \t" << -sensed_force_global.transpose() << std::endl;
			// std::cout << "end effector sensor moments global : \t" << -sensed_moment_global.tail(3).transpose() << std::endl;
			// std::cout << std::endl;
		}
